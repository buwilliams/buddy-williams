...
My children love Minecraft. Some players have built working computers inside it, out of the game's own blocks. That means there is a computer running a game, and inside the game, another computer. I think this fact, which most of us have seen and shrugged at, holds a lesson about the nature of reality.
...
1. [Layers and Substrates](#1-layers-and-substrates)
2. [Computers Inside Computers](#2-computers-inside-computers)
3. [Simulations and Observer Locality](#3-simulations-and-observer-locality)
4. [People Can Be Programs](#4-people-can-be-programs)
5. [Closed Causal Systems](#5-closed-causal-systems)
6. [Go Down to Go Up](#6-go-down-to-go-up)
...
Good thinking often means separating ideas into layers. Three kinds matter here.

An **explanatory layer** is a level of description matched to a question. Thermodynamics explains how a gas behaves without tracking every molecule; chemistry explains reactions without tracking every atom. We pick the layer that makes the question answerable.
...
What makes this possible is logic. Every computation can be built from a few basic operations: AND, OR, and NOT. A single gate, NAND, can reproduce all three, which is why modern chips are built largely from it. Once Minecraft gained Redstone, a way to build logic gates from blocks, it became possible to build a computer inside it. And there is no principled limit to the nesting. Each layer runs slower and needs resources from the one hosting it, but as long as logic is preserved, computers can contain computers that contain computers.
...
Put the pieces together. Programs can be computers. Computers can run simulations. People can be programs. So people can exist inside simulations. A person inside a simulation is still a person, and their experiences are real to them. That a hosting layer exists doesn't make the observer less real. It only says where the observer sits.

This closes a gap in the previous section. The observer is not a mysterious extra added to a simulation. It is itself a computational pattern running on the simulation's substrate: a program inside a program.
...
Now the key move. From the hosting layer, the rules can be anything. A program can include magic, time travel, or violations of conservation of energy. The programmer writes whatever rules they want. So the same ruleset has two faces. The observer inside sees necessity: the ball must bounce this way, the spell must cost this much mana, the character must die when health reaches zero. The observer outside sees a choice: it could have been written differently. Both are right. Necessity and contingency depend on where you stand.

Apply this to our own universe. At the scales we live in, it behaves as a closed causal system: effects follow from causes within the rules, and nothing we can detect leaks in from outside. There are open questions at the edges, quantum indeterminacy among them, but its behavior is describable as a ruleset that could in principle be run. Our universe shares two structural properties with every nested world we have built: it processes information, and it runs by rules that compose.
...
We began with a question about what is fundamental. Three candidates compete: God, physical matter, and computation. Ask each the same question: what explains you?
