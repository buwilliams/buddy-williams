...
There is a Popperian quality to this, at least in how I read it. Popper argued that we can't predict the future growth of knowledge, since predicting a discovery would mean already having it. The capacity forecast respects that limit. It doesn't claim to know how intelligence will emerge. It makes a weaker, more defensible claim: given more compute, the ordinary process of conjecture and criticism will keep finding more powerful designs, even though we can't name them in advance. The particular path is unpredictable. The direction is not. That asymmetry is the whole trick.

An objection: cheaper compute is itself a product of new knowledge, in chip design, lithography, and packaging, so doesn't forecasting it mean predicting discoveries after all? It doesn't. The capacity forecast doesn't need to predict any discovery. It rests on a regularity, and here I lean on induction more than Popper would: compute per dollar has improved on a steady cadence for decades, across many changes in the underlying technology. And the regularity comes with an explanation. Demand for compute funds the search that makes compute cheaper, and cheaper compute widens the demand. Induction on a regularity like that, backed by a mechanism, is how most of our knowledge works. Like any regularity, it could break, which is why the forecast is held fallibly and the method watches for the break ([Section 7](#7-where-the-idea-breaks-down) takes up how).
